...
//  ChrBuf = (BYTE *)malloc(256 * 2 * 8 * 8 * sizeof(BYTE));
//  RAM  = (BYTE *)malloc(RAM_SIZE * sizeof(BYTE));
//  SRAM = (BYTE *)malloc(SRAM_SIZE * sizeof(BYTE));

//  if (!WorkFrame)     
//   rt_kprintf("workframe malloc failed!\n");

  // int       i;
  // free(WorkFrame);

  /*-------------------------------------------------------------------*/
  /*  Initialize nes framebuffer                                       */
  /*-------------------------------------------------------------------*/
  ...
  // {
    ...
    start_application( "/nes/super_mali.nes" );
  // }
...
//   gdk_threads_leave();
  ...
  // lv_img_dsc_t nes_buffer = {
  //   .header.always_zero = 0,
  //   .header.w = NES_DISP_WIDTH,
  //   .header.h = NES_DISP_HEIGHT,
  //   .data_size = NES_DISP_WIDTH * NES_DISP_HEIGHT * LV_COLOR_SIZE / 8,
  //   .header.cf = LV_IMG_CF_TRUE_COLOR,
  //   .data = (uint8_t *)WorkFrame,
  // };
  // nes_fb_img_src.data = ;
  // lv_obj_t * img1 = lv_img_create(lv_scr_act(), NULL);
  // lv_img_set_src(img1, &nes_buffer);
