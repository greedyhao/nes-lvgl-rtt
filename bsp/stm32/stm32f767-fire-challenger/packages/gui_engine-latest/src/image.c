...
struct rtgui_image *rtgui_image_create_from_file(const char *type, const char *filename, rt_bool_t load)
{
    ...
    filerw = rtgui_filerw_create_file(filename, O_RDONLY);
    if (filerw == RT_NULL) 
    {
        rt_kprintf("image open failed!\n");
        return RT_NULL;
    }
    ...
    if (engine == RT_NULL)
    {
        ...
        rt_kprintf("engine image didnt found!\n");
    ...
    if (engine->image_check(filerw) == RT_TRUE)
    {
        ...
        {
            /* close filerw context */
            rt_kprintf("image malloc failed!\n");
            rtgui_filerw_close(filerw);
            return RT_NULL;
        }
        ...
        {
            /* close filerw context */
            rt_kprintf("engine image load failed!\n");
            rtgui_filerw_close(filerw);
            return RT_NULL;
        }
    ...
    {
        rt_kprintf("engine image check failed!\n");
        rtgui_filerw_close(filerw);
    }
