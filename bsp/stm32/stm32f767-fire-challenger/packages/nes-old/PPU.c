...
// 显示相关 
rt_uint8_t 	SpriteHitFlag, PPU_Latch_Flag; 	//sprite #0 显示碰撞点所在扫描行号, 背景位移￥2005写入标志 
int	PPU_scanline;  					//当前扫描行
rt_uint16_t	*Buffer_scanline;				//[8 + 256 + 8];	//行显示缓存,上下标越界最大为7，显示区 7 ~ 263  0~7 263~270 为防止溢出区
...
//PPU 初始化	 
void PPU_Init(	rt_uint8_t* patterntableptr,	//Pattern table 地址 
				rt_uint8_t  ScreenMirrorType	//屏幕镜像类型 
				)
{
	rt_memset(&PPU_Mem, 0, sizeof(PPU_Mem));//清零存储器
...
void PPU_MemWrite(rt_uint8_t value)
{
	switch(PPU_Mem.PPU_addrcnt & 0xF000)
	{
		...
		case 0x3000: 
			...
			}else PPU_Mem.image_palette[(PPU_Mem.PPU_addrcnt & 0xF)] = value; 	//背景颜色索引值表  
...
void NES_GetSpr0HitFlag(int y_axes)
{
	...
	dy_axes = y_axes - (rt_uint8_t)(sprite[0].y + 1);			//判断sprite#0 是否在当前行显示范围内,0坐标实际值为FF
	...
	if(x_TitleLine > 31)nNameTable ^= NAME_TABLE_H_MASK;//x轴跨屏显示	    
	...
	if((x_TitleLine + 1) > 31)nNameTable ^= NAME_TABLE_H_MASK;//x轴跨屏显示
	...
	BG_Data = (BG_Data0 << dx_axes) | (BG_Data1 >> dx_axes);							//背景与Sprite #0 位置相同的当前显示行的显示数据
 	//取得sprite #0 显示数据 
	if(sprite[0].attr & SPR_VFLIP)dy_axes = spr_size - dy_axes;//若垂直翻转	  
...
void NES_RenderBGLine(int y_axes)
{
	...
	if(y_scroll > 255)						//垂直超出屏幕，切换name table表
	{
		//y_scroll -= 240;
		y_scroll -= 256;
	...
	{		   
		...
		{							   
			...
			if(BG_color_num & 3)Buffer_scanline[Buffer_LineCnt] =  NES_Color_Palette[PPU_Mem.image_palette[BG_color_num]];//如果低两位为0，则为透明色,不写入
			Buffer_LineCnt++; 
		}
	}
...
void NES_RenderSprite16(SpriteType *sprptr, int dy_axes)
{
	...
	{										
		if(dy_axes < 8)	 //sprite  title 奇数号
			NES_RenderSprPattern(sprptr, PPU_Mem.patterntable1, (sprptr -> t_num & 0xFE) << 4, (rt_uint8_t)dy_axes);	//上8*8
		else
			NES_RenderSprPattern(sprptr, PPU_Mem.patterntable1, sprptr -> t_num << 4, (rt_uint8_t)dy_axes & 7);		   	//下8*8
	}else
	{
		if(dy_axes < 8)	//sprite  title 偶数号
			NES_RenderSprPattern(sprptr, PPU_Mem.patterntable0, sprptr -> t_num << 4, (rt_uint8_t)dy_axes);			   	//上8*8
		else
			NES_RenderSprPattern(sprptr, PPU_Mem.patterntable0, (sprptr -> t_num | 1) << 4, (rt_uint8_t)dy_axes & 7);   //下8*8
	}
}
...
void NES_RenderLine(int y_axes)
{
	...
	{					
		...
		for(i=7; i<256+8 ; i++)//显示区 7 ~ 263  0~7 263~270 为防止溢出区
		{						
		...
		{	
			render_spr_num=0;											//清零显示计数器
		 	for(i=63; i>=0; i--){										//若重叠sprites 0 具有显示最高优先级，其余优先级顺序次之，所以最先显示最低优先级
				...
				//判断显示位置 
				dy_axes = y_axes - (rt_uint8_t)(sprite[i].y + 1);				//判断sprite是否在当前行显示范围内,sprite y (FF,00,01,...EE)(0~239)
				...
				if(PPU_Reg.NES_R0 & R0_SPR_SIZE)NES_RenderSprite16(&sprite[i], dy_axes);//若为真，sprite的大小8*16
			    else NES_RenderSprite88(&sprite[i], dy_axes);//若为假，sprite的大小8*8
			}	
		}	  
		//扫描背景 background 
		if(PPU_Reg.NES_R1 & R1_BG_VISIBLE)NES_RenderBGLine(y_axes);//扫描并设置Sprite #0碰撞标志 	
    ...
    // else for(i=8; i<264; i++)Buffer_scanline[i] = black;//清空显示缓存,黑屏	 
	else for(i=7; i<264; i++)Buffer_scanline[i] = black;//清空显示缓存,黑屏	 
	//完成扫描，将行显示缓存写入LCD*/
	// NES_LCD_DisplayLine(y_axes, Buffer_scanline);		 //启动LCD显示一行，查询或DMA传送
	// memcpy(nes_fb_img_src.data + (y_axes*800), Buffer_scanline, 256);
	memcpy(nes_fb_img_src.data + (y_axes*1600), Buffer_scanline, 512);
