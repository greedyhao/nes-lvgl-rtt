...
extern uint16_t nes_xoff;			//显示在x轴方向的偏移量(实际显示宽度=256-2*nes_xoff)
extern uint16_t nes_yoff;			//显示在y轴方向的偏移量

extern uint32_t nes_rgb_parm1;
extern uint16_t nes_rgb_parm2;
extern uint16_t nes_rgb_parm3;
...
void scanline_draw(int LineNo)
{
	...
	uint32_t temp;
	uint32_t color=0;
	// uint32_t *plcd=ltdc_framebuf[lcdltdc.activelayer];	//指向显存地址减少数组访问,提高速度
	...
	// 	if(lcddev.id==0X4342)
	// 	{
	// 		temp=nes_rgb_parm1+nes_rgb_parm2*sx+(nes_yoff<<1)+(LineNo<<1);
	// 		for(i=sx;i<ex;i++)
	// 		{  
	// 			//offset=lcdltdc.pixsize*(lcdltdc.pwidth*(lcdltdc.pheight+(sx-i)-1)+nes_yoff+LineNo) 
	// 			//offset=2*(800*(480+(sx-i)*2-1)+nes_yoff+LineNo)
	// 			//      =1600*(480+(sx-i)*2-1)+2*nes_yoff+LineNo*2
	// 			//      =766400+3200*(sx-i)+2*nes_yoff+LineNo*2
	// 			offset=temp-nes_rgb_parm2*i;  
	// 			*(uint16_t*)((uint32_t)plcd+offset)=NES_Palette[ppu->dummy_buffer[i]];//得到颜色值	
	// 		} 
	// 	}else
	// 	{
	// 		temp=nes_rgb_parm1+nes_rgb_parm2*sx+(nes_yoff<<1)+(LineNo<<2);
	// 		for(i=sx;i<ex;i++)
	// 		{  
	// 			//offset=lcdltdc.pixsize*(lcdltdc.pwidth*(lcdltdc.pheight+(sx-i)*2-1)+nes_yoff+LineNo*2) 
	// 			//offset=2*(800*(480+(sx-i)*2-1)+nes_yoff+LineNo*2)
	// 			//      =1600*(480+(sx-i)*2-1)+2*nes_yoff+LineNo*4
	// 			//      =766400+3200*(sx-i)+2*nes_yoff+LineNo*4 
	// 			offset=temp-nes_rgb_parm2*i; 
	// 			color=NES_Palette[ppu->dummy_buffer[i]];//得到颜色值
	// 			color|=color<<16; 
	// 			*(uint32_t*)((uint32_t)plcd+offset)=color;		 
	// 			offset=offset-nes_rgb_parm3;
	// 			*(uint32_t*)((uint32_t)plcd+offset)=color;
	// 		} 
	// 	}			
	// }else	//MCU屏
