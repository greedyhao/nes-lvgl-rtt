...
//6502 memory map  
rt_uint8_t *ram6502;  			//RAM  2K字节,由malloc申请
//uint8_t* ppu_regbase;		// PPU IO reg 
//uint8_t* apu_regbase;		// APU IO reg 
rt_uint8_t* exp_rom;	    	// expansion rom 
rt_uint8_t* sram;				// sram 
rt_uint8_t* prg_rombank0;		// prg-rom lower bank 
rt_uint8_t* prg_rombank1;		// prg-rom upper bank 
...
{
	rt_uint8_t *scr_addrptr = 0;		//将传送sprite数据 指针
	int 	i;	  
...
//初始化6502 存储器 
void init6502mem( rt_uint8_t* exp_romptr,
				  rt_uint8_t* sramptr,			
				  rt_uint8_t* prg_rombank0ptr,		
			   	  rt_uint8_t  rom_num)		
{
